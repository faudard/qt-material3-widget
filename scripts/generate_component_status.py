...
CPP_REGISTRY = ROOT / "src" / "tooling" / "qtmaterialcomponentregistry_generated.inc"
...
def render_cpp_registry(components: list[dict[str, Any]]) -> str:
    lines = [
        "// Generated from docs/components/component-registry.json",
        "// by scripts/generate_component_status.py.",
    ...
    for item in components:
        ...
        fields = [
            cpp_string_literal(item.get("id", "")),
            cpp_string_literal(item.get("name", "")),
            cpp_string_literal(item.get("family", "")),
            cpp_string_literal(item.get("publicHeader", "")),
            cpp_string_literal("QtMaterial3::Widgets"),
            cpp_string_literal(item.get("specType", "")),
            cpp_string_literal(item.get("widgetType", "")),
            cpp_string_literal(item.get("docsPath", "")),
            cpp_string_literal(item.get("testTarget", "")),
            cpp_string_literal(item.get("galleryRoute", "")),
            "ComponentMaturity::" + CPP_MATURITY[maturity],
        ]
...
def main(argv: list[str]) -> int:
    ...
    outputs = {
        ...
        CPP_REGISTRY: render_cpp_registry(components),
    }
