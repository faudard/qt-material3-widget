...
void QtMaterialIconButton::paintEvent(QPaintEvent* event)
{
    ...
    const QColor containerColor =
        blendColor(m_spec.containerColor, m_spec.selectedContainerColor, progress);

    const QColor iconColor = isEnabled()
                                 ? blendColor(m_spec.iconColor, m_spec.selectedIconColor, progress)
                                 : m_spec.disabledIconColor;

    painter.fillPath(m_cachedContainerPath, containerColor);

    const qreal overlayOpacity =
        stateLayerOpacityFor(interactionState(), theme(), isEnabled());
