...
QtMaterialControl::QtMaterialControl(QWidget* parent)
    : QtMaterialWidget(parent)
    , m_state()
    , m_density(Density::Default)
{
    setAttribute(Qt::WA_Hover, true);
    setMaterialComponent(QStringLiteral("Control"));
    syncAutomationState();
}
...
void QtMaterialControl::setDensity(Density density)
{
...
const QtMaterialInteractionState& QtMaterialControl::interactionState() const noexcept
{
    return m_state;
}

QtMaterialInteractionState& QtMaterialControl::interactionState() noexcept
{
    return m_state;
}
...
void QtMaterialControl::stateChangedEvent()
{
    syncAutomationState();
    update();
}
...
void QtMaterialControl::leaveEvent(QEvent* event)
{
    ...
    stateChangedEvent();
    QtMaterialWidget::leaveEvent(event);
}
...
void QtMaterialControl::focusOutEvent(QFocusEvent* event)
{
    m_state.setFocused(false);
    m_state.setPressed(false);
    stateChangedEvent();
    QtMaterialWidget::focusOutEvent(event);
}

void QtMaterialControl::changeEvent(QEvent* event)
{
