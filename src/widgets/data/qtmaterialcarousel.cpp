...
bool isValidIndex(const QtMaterialCarouselPrivate* d, int index) noexcept
{
    return d && index >= 0 && index < d->m_items.size();
}

int firstVisibleIndex(const QtMaterialCarouselPrivate* d) noexcept
{
    if (!d || d->m_items.isEmpty()) {
        return 0;
    }

    const int half = d->m_visibleItemCount / 2;
    return qBound(0,
                  d->m_currentIndex - half,
                  qMax(0, d->m_items.size() - d->m_visibleItemCount));
}

int nextEnabledIndex(const QtMaterialCarouselPrivate* d, int start, int delta) noexcept
{
    if (!d || d->m_items.isEmpty() || delta == 0) {
        return -1;
    }

    int index = isValidIndex(d, start) ? start : (delta > 0 ? -1 : d->m_items.size());
    for (int step = 0; step < d->m_items.size(); ++step) {
        index += delta;
        if (d->m_wrapAround) {
...
QRect itemRect(const QtMaterialCarousel* widget, const QtMaterialCarouselPrivate* d, int index)
{
    if (!widget || !isValidIndex(d, index)) {
        return QRect();
    }

    const int first = firstVisibleIndex(d);
    const int last = qMin(d->m_items.size(), first + d->m_visibleItemCount);
    ...
    const int availableWidth = widget->width() - kPadding * 2 - qMax(0, cards - 1) * kGap;
    const int cardWidth = qMax(72, availableWidth / qMax(1, cards));
    const int cardHeight = qMax(72, widget->height() - kPadding * 2);
    const int visualOffset = widget->layoutDirection() == Qt::RightToLeft ? cards - 1 - visibleOffset : visibleOffset;
    const int x = kPadding + visualOffset * (cardWidth + kGap);

    return QRect(x, kPadding, cardWidth, cardHeight);
}

void updateAccessibilitySummary(QtMaterialCarousel* widget, QtMaterialCarouselPrivate* d)
{
    if (!widget || !d) {
        return;
    }
    ...
    if (d->m_items.isEmpty()) {
        ...
        summary = QStringLiteral("Carousel, %1 items, current %2")
                      .arg(d->m_items.size())
                      .arg(widget->currentItemAccessibleText());
    }
    ...
    widget->setAccessibleDescription(d->m_accessibilitySummary);
    emit widget->accessibilitySummaryChanged(d->m_accessibilitySummary);
}

void emitCurrentIndexChanged(QtMaterialCarousel* widget, int index)
{
    if (!widget) {
        return;
    }
    emit widget->currentIndexChanged(index);
}
    ...
    : QWidget(parent)
{
    ...
    updateAccessibilitySummary(this, d_ptr.get());
}
...
void QtMaterialCarousel::insertItem(int index, const QString& title, const QString& supportingText, const QIcon& icon)
{
    ...
    if (d_ptr->m_currentIndex < 0) {
        ...
        emitCurrentIndexChanged(this, d_ptr->m_currentIndex);
...
void QtMaterialCarousel::removeItem(int index)
{
    ...
    if (d_ptr->m_items.isEmpty()) {
        ...
        if (!d_ptr->m_items.at(d_ptr->m_currentIndex).enabled) {
            const int enabled = nextEnabledIndex(d_ptr.get(), d_ptr->m_currentIndex, 1);
...
{
    return isValidIndex(d_ptr.get(), index) ? d_ptr->m_items.at(index).title : QString();
}

void QtMaterialCarousel::setItemTitle(int index, const QString& title)
{
    if (!isValidIndex(d_ptr.get(), index) || d_ptr->m_items[index].title == title) {
        return;
    }
...
{
    return isValidIndex(d_ptr.get(), index) ? d_ptr->m_items.at(index).supportingText : QString();
}

void QtMaterialCarousel::setItemSupportingText(int index, const QString& text)
{
    if (!isValidIndex(d_ptr.get(), index) || d_ptr->m_items[index].supportingText == text) {
        return;
    }
...
{
    return isValidIndex(d_ptr.get(), index) && d_ptr->m_items.at(index).enabled;
}

void QtMaterialCarousel::setItemEnabled(int index, bool enabled)
{
    if (!isValidIndex(d_ptr.get(), index) || d_ptr->m_items[index].enabled == enabled) {
        return;
    }

    d_ptr->m_items[index].enabled = enabled;
    if (!enabled && index == d_ptr->m_currentIndex) {
        const int replacement = nextEnabledIndex(d_ptr.get(), index, 1);
        if (replacement >= 0) {
...
{
    if (!isValidIndex(d_ptr.get(), index) || !d_ptr->m_items.at(index).enabled || index == d_ptr->m_currentIndex) {
        return;
    }
...
{
    const int nextIndex = nextEnabledIndex(d_ptr.get(), d_ptr->m_currentIndex, 1);
    if (nextIndex >= 0) {
...
{
    const int previousIndex = nextEnabledIndex(d_ptr.get(), d_ptr->m_currentIndex, -1);
    if (previousIndex >= 0) {
...
{
    if (isValidIndex(d_ptr.get(), d_ptr->m_currentIndex) && d_ptr->m_items.at(d_ptr->m_currentIndex).enabled) {
...
void QtMaterialCarousel::mouseReleaseEvent(QMouseEvent* event)
{
    ...
    for (int i = 0; i < d_ptr->m_items.size(); ++i) {
        if (itemRect(this, d_ptr.get(), i).contains(event->pos()) && d_ptr->m_items.at(i).enabled) {
            setCurrentIndex(i);
            emit itemClicked(i);
...
void QtMaterialCarousel::paintEvent(QPaintEvent*)
{
    ...
    for (int i = 0; i < d_ptr->m_items.size(); ++i) {
        const QRect rect = itemRect(this, d_ptr.get(), i);
        if (rect.isNull()) {
            continue;
        }
