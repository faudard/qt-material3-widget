...
ThemeBuilder::~ThemeBuilder() = default;
Theme ThemeBuilder::build(const ThemeOptions& options) const { return buildFallbackTheme(options); }
Theme ThemeBuilder::buildLightFromSeed(const QColor& seed) const { ThemeOptions options; options.sourceColor = seed; options.mode = ThemeMode::Light; return build(options); }
Theme ThemeBuilder::buildDarkFromSeed(const QColor& seed) const { ThemeOptions options; options.sourceColor = seed; options.mode = ThemeMode::Dark; return build(options); }
Theme ThemeBuilder::buildFallbackTheme(const ThemeOptions& options) const
{
    Theme theme(options);
    if (options.mode == ThemeMode::Dark) applyFallbackDarkScheme(theme, options.sourceColor);
    else applyFallbackLightScheme(theme, options.sourceColor);
